#!/usr/bin/env python
"""Extract and index DOCX figures into Chroma."""
...
from rag.figure_extractor import extract_figures_from_file, index_figure_records
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Extract and index figures from local 3GPP DOCX documents."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Report figure indexing targets without writing images, SQLite, Chroma, or embeddings.",
    )
    ...
    for path, metadata in documents:
        try:
            if args.dry_run:
                print(f"[dry-run] Would extract and index figures from {path}")
                continue
            ...
            indexed = index_figure_records(figures)
            figures_total += len(figures)
            figures_indexed += indexed
            print(f"Indexed {indexed}/{len(figures)} figure(s) from {path}")
        except Exception as exc:
            failed += 1
            print(f"Failed to index figures from {path}: {exc}")

    print(
        "Figure indexing complete: "
        f"documents={len(documents)}, figures_total={figures_total}, "
        f"figures_indexed={figures_indexed}, failed={failed}"
    )
